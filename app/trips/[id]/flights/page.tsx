...
export default function FlightBookingPage() {
  ...
  const handleBookFlight = async (bookingData: FlightBookingData) => {
    ...
    try {
      const endpoint = tripId ? `/api/trips/${tripId}/flights` : '/api/flights'
      ...
      if (data.success) {
        ...
        // Refresh bookings if we're on a trip page
        if (tripId) {
          fetchTripFlights()
        }
  ...
  const handleCancelBooking = async (bookingId: string) => {
    ...
    try {
      const endpoint = tripId ? `/api/trips/${tripId}/flights?bookingId=${bookingId}` : `/api/flights/${bookingId}`
      const response = await fetch(endpoint, {
        method: 'DELETE'
      })
          ...
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
            <div>
              <h1 className="text-2xl font-bold text-slate-900 dark:text-white">
                {currentTrip ? `Flights for ${currentTrip.name}` : 'Book Your Flights'}
              </h1>
              <p className="text-slate-500 dark:text-slate-400 mt-1">
                {currentTrip 
                  ? `Find the best deals for your ${currentTrip.name}`
            ...
            <div className="flex gap-3">
              ...
              {tripId && (
                <motion.button
                  ...
                  className="px-5 py-2 bg-primary hover:bg-primary-dark text-white rounded-lg text-sm font-medium shadow-sm transition"
                >
                  Save Trip
                </motion.button>
              )}
